...
def main(args):
    ...
    print(f"dump: {dump_only}")
    print(f"osp: {osp.isfile(args.video)}")
    print(f"os: {os.path.isfile(args.video)}")
    ...
    if args.video == "webcam":
        ...
        vs.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'))
    ...
    if args.output:
        ...
        # save as a single video file
            ...
            bbox_result = open(args.output.split(".mp4")[0]+'.csv', 'w')
            w = csv.writer(bbox_result)
            vw = VideoWriter(args.output, fps=20)
    
    
    print(f"isOpen: {vs.isOpened()}")
    # loop over sequence
    ...
    while vs.isOpened():
        ...
        if ret:
            ...
            if template is not None:
                ...
                show_frame[:128,:128]=cv2.resize(frame[int(bbox_pred[1]):int(bbox_pred[3]), int(bbox_pred[0]):int(bbox_pred[2])],(128, 128))
            else:
                show_frame = frame
...
if __name__ == "__main__":
    ...
    print(f"os: {os.path.exists(args.video)}")
    main(args)
